...
# Exclude high-cardinality ID column 'flight' and index
drop_cols = [c for c in ["Unnamed: 0", "flight"] if c in df.columns]
df_clean = df.drop(columns=drop_cols)

print(f"Dimensiones tras excluir identificador 'flight': {df_clean.shape}")

# Generate EDA categorical plot
...
# Split strictly: Train (70%), Val (15%), Test (15%)
...
print("\n=========================================================")
print("=== 4. EVALUACION DE 3 ARQUITECTURAS DE DEEP LEARNING   ===")
print("=========================================================")
...
for idx, (arch_name, config) in enumerate(dl_architectures.items()):
    ...
    val_mse = mean_squared_error(y_val_raw, preds_val_unscaled)
    ...
    ax.plot(epochs_range, val_loss, label=f"{arch_name} (Val Loss)", color=colors[idx], linewidth=2)
    ...
    dl_tuning_results.append(
        {
            ...
            "Train_MSE_Scaled": round(float(train_loss[-1]), 4),
            "Val_MSE_Rupees": round(float(val_mse), 2),
            "Val_RMSE_Rupees": round(float(val_rmse), 2),
...
ax.set_title("Curvas de Convergencia de Pérdida (MSE) por Época — Arquitecturas Deep Learning", fontsize=13, fontweight="bold")
ax.set_xlabel("Épocas", fontsize=11)
ax.set_ylabel("Pérdida MSE Escala Z", fontsize=11)
...
print("[OK] Comparación de las 3 arquitecturas de Deep Learning completada.")
...
# Evaluate DL Standard model on Test Set
...
print("\n=== RESULTADOS FINALES EN EL CONJUNTO DE PRUEBA (TEST SET) ===")
...
print("\n=========================================================")
print("=== 6. CALCULO DE EXPLICABILIDAD SHAP                   ===")
print("=========================================================")
...
try:
    ...
    print(f"Warning SHAP: {e}. Usando importancias directas del Random Forest...")
...
print("\n=========================================================")
print("=== ¡EXPERIMENTOS S6 COMPLETADOS CON EXITO!           ===")
print("=========================================================")
